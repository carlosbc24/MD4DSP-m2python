...
class DataSmellsSimpleTest(unittest.TestCase):
    ...
    def execute_check_ambiguous_datetime_format_SimpleTests(self):
        ...
        # Test 1: Format with 12-hour pattern "%I:%M %p" (smell)
        df1 = pd.DataFrame({'datetime_col': ['2025-06-26 02:30 PM', '2025-06-26 09:15 AM']})
        result = self.data_smells.check_ambiguous_datetime_format(df1, 'datetime_col', '%m/%d/%Y %I:%M %p')
        self.assertFalse(result, "Test Case 1 Failed: Expected smell for %I:%M %p format")
        print_and_log("Test Case 1 Passed: Expected smell, got smell")

        # Test 2: Format with 24-hour pattern (no smell)
        df2 = pd.DataFrame({'datetime_col': ['2025-06-26 14:30:00', '2025-06-26 09:15:30']})
        result = self.data_smells.check_ambiguous_datetime_format(df2, 'datetime_col', '%Y-%m-%d %H:%M:%S')
        ...
        # Test 3: Exact pattern "%I:%M %p" (smell)
        df3 = pd.DataFrame({'time_col': ['02:30 PM', '09:15 AM']})
        result = self.data_smells.check_ambiguous_datetime_format(df3, 'time_col', '%I:%M %p')
        self.assertFalse(result, "Test Case 3 Failed: Expected smell for exact %I:%M %p pattern")
        print_and_log("Test Case 3 Passed: Expected smell, got smell")

        # Test 4: Date-only format (no smell)
        df4 = pd.DataFrame({'date_col': ['2025-06-26', '2025-12-31']})
        result = self.data_smells.check_ambiguous_datetime_format(df4, 'date_col', '%Y-%m-%d')
        self.assertTrue(result, "Test Case 4 Failed: Expected no smell for date-only format")
        print_and_log("Test Case 4 Passed: Expected no smell, got no smell")

        # Test 5: Non-existent field
        with self.assertRaises(ValueError):
            self.data_smells.check_ambiguous_datetime_format(df1, 'non_existent_field', '%Y-%m-%d')
        print_and_log("Test Case 5 Passed: Expected ValueError for non-existent field")

        # Test 6: Complex format with 12-hour pattern (smell)
        df6 = pd.DataFrame({'datetime_col': ['Monday, June 26, 2025 at 2:30 PM']})
        result = self.data_smells.check_ambiguous_datetime_format(df6, 'datetime_col', '%A, %B %d, %Y at %I:%M %p')
        self.assertFalse(result, "Test Case 6 Failed: Expected smell for complex format with %I:%M %p")
        print_and_log("Test Case 6 Passed: Expected smell, got smell")

        # Test 7: Format with %I but no %p (no smell - not exact pattern)
        df7 = pd.DataFrame({'time_col': ['14', '09']})
        result = self.data_smells.check_ambiguous_datetime_format(df7, 'time_col', '%I')
        self.assertTrue(result, "Test Case 7 Failed: Expected no smell for %I without %p")
        print_and_log("Test Case 7 Passed: Expected no smell, got no smell")

        # Test 8: Format with %p but no %I (no smell - not exact pattern)
        df8 = pd.DataFrame({'time_col': ['PM', 'AM']})
        result = self.data_smells.check_ambiguous_datetime_format(df8, 'time_col', '%p')
        self.assertTrue(result, "Test Case 8 Failed: Expected no smell for %p without %I")
        print_and_log("Test Case 8 Passed: Expected no smell, got no smell")

        # Test 9: Empty format string (error)
        with self.assertRaises(ValueError):
            self.data_smells.check_ambiguous_datetime_format(df1, 'datetime_col', '')
        print_and_log("Test Case 9 Passed: Expected ValueError for empty format string")

        # Test 10: Format with seconds in 12-hour pattern (no smell - not exact "%I:%M %p" pattern)
        df10 = pd.DataFrame({'datetime_col': ['02:30:45 PM', '09:15:30 AM']})
        result = self.data_smells.check_ambiguous_datetime_format(df10, 'datetime_col', '%I:%M:%S %p')
        self.assertTrue(result, "Test Case 10 Failed: Expected no smell for %I:%M:%S %p format (not exact pattern)")
        print_and_log("Test Case 10 Passed: Expected no smell, got no smell")

        # Test 11: Format with timezone and 12-hour pattern (smell)
        df11 = pd.DataFrame({'datetime_col': ['2025-06-26 02:30 PM EST']})
        result = self.data_smells.check_ambiguous_datetime_format(df11, 'datetime_col', '%Y-%m-%d %I:%M %p %Z')
        self.assertFalse(result, "Test Case 11 Failed: Expected smell for format with timezone and %I:%M %p")
        print_and_log("Test Case 11 Passed: Expected smell, got smell")

        # Test 12: Format with timezone but 24-hour pattern (no smell)
        df12 = pd.DataFrame({'datetime_col': ['2025-06-26 14:30 EST']})
        result = self.data_smells.check_ambiguous_datetime_format(df12, 'datetime_col', '%Y-%m-%d %H:%M %Z')
        self.assertTrue(result, "Test Case 12 Failed: Expected no smell for 24-hour format with timezone")
        print_and_log("Test Case 12 Passed: Expected no smell, got no smell")

        # Test 13: Multiple datetime components with 12-hour pattern (smell)
        df13 = pd.DataFrame({'datetime_col': ['26/06/2025 02:30 PM - 26/06/2025 03:30 PM']})
        result = self.data_smells.check_ambiguous_datetime_format(df13, 'datetime_col', '%d/%m/%Y %I:%M %p - %d/%m/%Y %I:%M %p')
        self.assertFalse(result, "Test Case 13 Failed: Expected smell for multiple %I:%M %p patterns")
        print_and_log("Test Case 13 Passed: Expected smell, got smell")

        # Test 14: Non-string format parameter (error)
        with self.assertRaises(ValueError):
            self.data_smells.check_ambiguous_datetime_format(df1, 'datetime_col', 123)
        print_and_log("Test Case 14 Passed: Expected ValueError for non-string format")

        # Test 15: Whitespace-only format string (error)
        with self.assertRaises(ValueError):
            self.data_smells.check_ambiguous_datetime_format(df1, 'datetime_col', '   ')
        print_and_log("Test Case 15 Passed: Expected ValueError for whitespace-only format")

        # Test 16: Format with %I but missing %M (no smell - not exact pattern)
        df16 = pd.DataFrame({'time_col': ['02 PM', '09 AM']})
        result = self.data_smells.check_ambiguous_datetime_format(df16, 'time_col', '%I %p')
        self.assertTrue(result, "Test Case 16 Failed: Expected no smell for %I %p without %M")
        print_and_log("Test Case 16 Passed: Expected no smell, got no smell")

        # Test 17: Format with %M but missing %I (no smell - not exact pattern)
        df17 = pd.DataFrame({'time_col': ['30 PM', '15 AM']})
        result = self.data_smells.check_ambiguous_datetime_format(df17, 'time_col', '%M %p')
        self.assertTrue(result, "Test Case 17 Failed: Expected no smell for %M %p without %I")
        print_and_log("Test Case 17 Passed: Expected no smell, got no smell")

        # Test 18: Format with reversed pattern %p %M:%I (no smell - not exact pattern)
        df18 = pd.DataFrame({'time_col': ['PM 30:02', 'AM 15:09']})
        result = self.data_smells.check_ambiguous_datetime_format(df18, 'time_col', '%p %M:%I')
        self.assertTrue(result, "Test Case 18 Failed: Expected no smell for reversed pattern")
        print_and_log("Test Case 18 Passed: Expected no smell, got no smell")

        # Test 19: Case sensitivity test - lowercase pattern (no smell)
        df19 = pd.DataFrame({'time_col': ['02:30 pm', '09:15 am']})
        result = self.data_smells.check_ambiguous_datetime_format(df19, 'time_col', '%i:%m %p')
        self.assertTrue(result, "Test Case 19 Failed: Expected no smell for lowercase pattern")
        print_and_log("Test Case 19 Passed: Expected no smell, got no smell")

        # Test 20: Multiple occurrences of exact pattern (smell detected)
        df20 = pd.DataFrame({'time_col': ['02:30 PM to 03:45 PM']})
        result = self.data_smells.check_ambiguous_datetime_format(df20, 'time_col', '%I:%M %p to %I:%M %p')
        self.assertFalse(result, "Test Case 20 Failed: Expected smell for multiple %I:%M %p occurrences")
        print_and_log("Test Case 20 Passed: Expected smell, got smell")

        # Test 21: Pattern within quoted text (smell detected)
        df21 = pd.DataFrame({'time_col': ['Time is 02:30 PM today']})
        result = self.data_smells.check_ambiguous_datetime_format(df21, 'time_col', 'Time is %I:%M %p today')
        self.assertFalse(result, "Test Case 21 Failed: Expected smell for pattern within text")
        print_and_log("Test Case 21 Passed: Expected smell, got smell")

        # Test 22: None as datetime_format parameter (error)
        with self.assertRaises(ValueError):
            self.data_smells.check_ambiguous_datetime_format(df1, 'datetime_col', None)
        print_and_log("Test Case 22 Passed: Expected ValueError for None format")

        # Test 23: Empty string as field parameter (error)
        with self.assertRaises(ValueError):
            self.data_smells.check_ambiguous_datetime_format(df1, '', '%I:%M %p')
        print_and_log("Test Case 23 Passed: Expected ValueError for empty field name")

        # Test 24: Format with milliseconds but not exact pattern (no smell - has seconds between)
        df24 = pd.DataFrame({'datetime_col': ['2025-06-26 02:30:45.123 PM']})
        result = self.data_smells.check_ambiguous_datetime_format(df24, 'datetime_col', '%Y-%m-%d %I:%M:%S.%f %p')
        self.assertTrue(result, "Test Case 24 Failed: Expected no smell for format with milliseconds (not exact pattern)")
        print_and_log("Test Case 24 Passed: Expected no smell, got no smell")

        # Test 25: International format with 12-hour pattern (smell detected)
        df25 = pd.DataFrame({'datetime_col': ['26/06/2025 14:30 PM']})
        result = self.data_smells.check_ambiguous_datetime_format(df25, 'datetime_col', '%d/%m/%Y %I:%M %p')
        self.assertFalse(result, "Test Case 25 Failed: Expected smell for international format")
