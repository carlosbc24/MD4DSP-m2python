...
class ContractsPrePost:
        ...
        if field is None:
            if belongOp == Belong.BELONG:
                if quant_op is None:  # Check if value is in dataDictionary
                    return True if value in dataDictionary.values else False # Case 1 y 2
                else:
                    if quant_rel is not None and quant_abs is None:  # Check if value is in dataDictionary and if it meets the condition of quant_rel
                        ...
                        return True if value in dataDictionary.values and compare_numbers( # Case 5 y 6
                ...
                if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                    return True if value not in dataDictionary.values else False  # Case 8 y 9
                else:
                    raise ValueError(
                        "Error: quant_rel and quant_abs should be None when belongOp is NOTBELONG")  # Case 10
        else:
            if field is not None:
                if field not in dataDictionary.columns:  # It checks that the column exists in the dataframe
                    raise ValueError(f"Column '{field}' not found in dataDictionary.")  # Case 10.5
                if belongOp == Belong.BELONG: # If the value should belong to the column
                    if quant_op is None:
                        return True if value in dataDictionary[field].values else False  # Case 11 y 12
                    else:
                        if quant_rel is not None and quant_abs is None:
                            return True if value in dataDictionary[field].values and compare_numbers(  # Case 13 y 14
                    ...
                    if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                        return True if value not in dataDictionary[field].values else False  # Case 18 y 19
                    else:  # Case 20
    ...
    def checkIntervalRangeFloat(self, left_margin: float, right_margin: float, dataDictionary: pd.DataFrame,
        ...
        if field is None:
            ...
            return check_condition(dataDictionary.min().min(),
                ...
                dataDictionary.min().min(), dataDictionary.max().max())  # Cases 1-16
        else:
            if field not in dataDictionary.columns:  # It checks that the column exists in the dataframe
                raise ValueError(f"Column '{field}' not found in dataDictionary.")  # Case 16.5
            if np.issubdtype(dataDictionary[field].dtype, np.number):
                return check_condition(dataDictionary[field].min(),
                    ...
                    dataDictionary[field].min(), dataDictionary[field].max())  # Cases 17-32
            else:
    ...
    def checkMissingRange(self, belongOp: Belong, dataDictionary: pd.DataFrame, field: str = None,
        ...
        if field is None:
            if belongOp == Belong.BELONG:
                if quant_op is None:  # Checks if there are any missing values from the list 'missing_values' in dataDictionary
                    if dataDictionary.isnull().values.any():
                        ...
                        if missing_values is not None:
                            return True if any(
                                ...
                                dataDictionary.values.flatten()) else False  # Case 2 y 3
                ...
                if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                    ...
                    if missing_values is not None:
                        return True if not dataDictionary.isnull().values.any() and not any(
                            value in missing_values for value in
                            dataDictionary.values.flatten()) else False  # Case 11 y 12
                    else:  # If the list is None, it checks that there aren't any python null values in dataDictionary
                        return True if not dataDictionary.isnull().values.any() else False  # Case 13 y 13.5
                else:
                    raise ValueError(
            ...
            if field is not None:  # Check to make code more legible
                if field not in dataDictionary.columns:  # Checks that the column exists in the dataframe
                    raise ValueError(f"Column '{field}' not found in dataDictionary.")  # Case 15
                if belongOp == Belong.BELONG:
                    if quant_op is None:  # Check that there are null python values or missing values from the list
                        ...
                        if dataDictionary[field].isnull().values.any():
                            ...
                            if missing_values is not None:
                                return True if any(
                                    value in missing_values for value in
                                    dataDictionary[field].values) else False  # Case 17 y 18
                            else:  # If the list is None, it returns False. It checks that in fact there aren't any missing values
                    ...
                    if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                        ...
                        if missing_values is not None:  # Check that there are missing values in the list 'missing_values'
                            return True if not dataDictionary[field].isnull().values.any() and not any(
                                value in missing_values for value in
                                dataDictionary[field].values) else False  # Case 26 y 27
                        else:  # If the list is None, it checks that there aren't any python null values in the column specified by field
                            return True if not dataDictionary[field].isnull().values.any() else False  # Case 28 y 29
                    else:
                        raise ValueError(
            ...
            if belongOp == Belong.BELONG:
                ...
                if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                    ...
                    return True if not (invalid_values is not None and any(
                        ...
                        dataDictionary.values.flatten())) else False  # Case 12 y 13
                else:
                    raise ValueError(
                    ...
                    if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                        ...
                        if invalid_values is not None: # Checks that there are invalid values in the list 'invalid_values'
                            return True if not any(
                                value in invalid_values for value in
                                dataDictionary[field].values) else False  # Case 27 y 28
    ...
    def checkOutliers(self, dataDictionary: pd.DataFrame, belongOp: Belong = None, field: str = None,
        ...
        if field is None:
            ...
            if belongOp == Belong.BELONG:
                ...
                if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                    return True if not (outlier in dataDictionary_copy.values) else False  # Case 9 y 10
                else:
            ...
            if field is not None:
                if field not in dataDictionary.columns:
                    raise ValueError(f"Column '{field}' not found in dataDictionary.")  # Case 12
                ...
                if belongOp == Belong.BELONG:
                    ...
                    if belongOp == Belong.NOTBELONG and quant_op is None and quant_rel is None and quant_abs is None:
                        # Check that there aren't any invalid values in the column specified by field
                        return True if not (outlier in dataDictionary_copy[field].values) else False  # Case 21 y 22
                    else:
                        raise ValueError(
